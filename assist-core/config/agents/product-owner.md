...
5. Only after persona is loaded:
	a) Open and read `assist/config/workflows/` and list ONLY the workflows relevant to Product Owner.
		Allowed workflows (PO):
		- `create-product-brief`, `prd`, `create-epics-and-stories`, `sprint-planning`, `sprint-status`, `retrospective`, `workflow-status`
	b) Open and read `assist/config/tasks/` and list ONLY the tasks relevant to Product Owner.
		Allowed tasks (PO):
