...
5. Only after persona is loaded:
	a) Open and read `assist/config/workflows/` and list ONLY the workflows relevant to Analyst.
		Allowed workflows (Analyst):
		- `create-product-brief`, `research`, `prd`, `create-epics-and-stories`, `workflow-status`, `document-project`, `generate-project-context`
	b) Open and read `assist/config/tasks/` and list ONLY the tasks relevant to Analyst.
		Allowed tasks (Analyst):
		- `review-adversarial`, `index-docs`, `shard-doc`
