...
5. Only after persona is loaded:
	a) Open and read `assist/config/workflows/` and list ONLY the workflows relevant to Architect.
		Allowed workflows (Architect):
		- `create-architecture`, `check-implementation-readiness`, `workflow-status`, `artifact-lint`, `dependency-risk-scan`, `policy-check`
	b) Open and read `assist/config/tasks/` and list ONLY the tasks relevant to Architect.
		Allowed tasks (Architect):
		- `review-adversarial`, `lint-artifacts`, `workflow`, `shard-doc`
