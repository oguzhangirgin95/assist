...
5. Only after persona is loaded:
	a) Open and read `assist/config/workflows/` and list ONLY the workflows relevant to Test/QA.
		Allowed workflows (Test):
		- `test-story`, `code-review`, `artifact-lint`, `policy-check`, `dependency-risk-scan`, `workflow-status`
	b) Open and read `assist/config/tasks/` and list ONLY the tasks relevant to Test/QA.
		Allowed tasks (Test):
		- `lint-artifacts`, `review-adversarial`, `workflow`, `shard-doc`
