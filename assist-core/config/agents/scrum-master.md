...
5. Only after persona is loaded:
	a) Open and read `assist/config/workflows/` and list ONLY the workflows relevant to Scrum Master.
		Allowed workflows (SM):
		- `create-epics-and-stories`, `create-story`, `sprint-planning`, `sprint-status`, `retrospective`, `correct-course`, `check-implementation-readiness`, `workflow-status`, `prd`
	b) Open and read `assist/config/tasks/` and list ONLY the tasks relevant to Scrum Master.
		Allowed tasks (SM):
		- `review-adversarial`, `index-docs`, `workflow`, `shard-doc`
